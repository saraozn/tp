...
This command allows users of the AddressBook application to add optional remarks to people in their propAddress book and edit it if required. The command should have the following format:
...
Let’s start by creating a new `RemarkCommand` class in the `src/main/java/seedu/propAddress/logic/command` directory.
...
``` java
package seedu.propAddress.logic.commands;

import seedu.propAddress.model.Model;

/**
 * Changes the remark of an existing person in the propAddress book.
 */
...
``` java
import static seedu.propAddress.commons.util.CollectionUtil.requireAllNonNull;
//...
public class RemarkCommand extends Command {
    //...
...
Create a `RemarkCommandParser` class in the `seedu.propAddress.logic.parser` package. The class must extend the `Parser` interface.
...
Now that we have all the information that we need, let’s lay the groundwork for propagating the remarks added into the in-memory storage of person data. We achieve that by working with the `Person` model. Each field in a Person is implemented as a separate class (e.g. a `Name` object represents the person’s propName). That means we should add a `Remark` class so that we can use a `Remark` object to represent a remark given to a person.

### Add a new `Remark` class

Create a new `Remark` in `seedu.propAddress.model.person`. Since a `Remark` is a field that is similar to `Address`, we can reuse a significant bit of code.
...
Simply add the following to [`seedu.propAddress.ui.PersonCard`](https://github.com/se-edu/addressbook-level3/commit/850b78879582f38accb05dd20c245963c65ea599#diff-639834f1e05afe2276a86372adf0fe5f69314642c2d93cfa543d614ce5a76688).
...
:exclamation: You must delete AddressBook’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause AddressBook to default to an empty propAddress book!
...
Following convention, let’s change the propName of the generated method to `execute_addRemarkUnfilteredList_success`.
